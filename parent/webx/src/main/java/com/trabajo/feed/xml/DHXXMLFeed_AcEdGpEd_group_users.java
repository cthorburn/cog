...
public class DHXXMLFeed_AcEdGpEd_group_users implements Feed {

	public String construct(TSession tsession, Engine eng, EntityManager em, FeedParms fp) {

		String groupName=((FeedParms_AcEdGpEd_group_users)fp).groupName();
	...
	//	TypedQuery<Object[]> q = em.createQuery("SELECT DISTINCT g.id, g.name, g.description, u.id from User u LEFT OUTER JOIN u.groups g WHERE u.id=?1 or g.id IS NOT NULL", Object[].class);
		Query q = em.createNativeQuery("select u.id, u.full_name, u.primaryemail, CASE WHEN gu.gu_user_id IS NULL  THEN 'false' ELSE 'true' END  from 	TGroup g inner join Group_users gu on g.id=gu.gu_group_id and g.GROUP_NAME=?1 right outer join TUSER u on u.id=gu.gu_user_id", Object[].class);
