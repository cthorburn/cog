...
public final class ExtURLClassloader<T extends CLMKey<T>> extends URLClassLoader {
    ...
    private final ClassLoaderManager<T, ?> clm;
    ...
    protected synchronized Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
        ...
        ClassLoader parent;
				try {
					...
					throw new ClassNotFoundException("repository exception");
				}
        ...
        try {
            ...
            if (fixUpList != null && fixUpList.contains(name)) {
                final ExtURLClassloader<T> orcl = getKeySpecificClassLoader(pti.key());
    ...
    public void close() throws IOException {
    		for(ExtURLClassloader<T> cl: overrideClassLoaders.values()) {
    			try {
    			cl.close();
    			}
    			catch(Exception e) {
