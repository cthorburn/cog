...
public class DHXXMLFeed_TkAvGd implements Feed {
	...
	public String construct(TSession tsession, Engine eng, EntityManager em, FeedParms fp) {
		...
		String jpql = "SELECT  NEW com.trabajo.feed.xml.DHXXMLFeed_TkAvGd$GridRow(n.id, n.name, pd.definitionVersion, n.owner.fullName, n.importance, n.dateAssigned, n.dateDue, i.note)"
				+ " FROM Node n INNER JOIN n.roles r INNER JOIN n.instance i INNER JOIN i.procDef pd  WHERE ?1 MEMBER OF r.users AND n.complete IS NULL AND NOT n.status=com.trabajo.TaskStatus.BARRIERED";

		

		TypedQuery<GridRow> q = em.createQuery(jpql, GridRow.class);

		q.setParameter(1, Users.findByName(em, tsession.getRemoteUser()).entity());
	...
	public static class GridRow {
		...
		public GridRow(int id, String name, VersionJPA version, String initName, String importance, GregorianCalendar dateAssigned, GregorianCalendar dateDue,
				String note) {
			super();
			this.id = id;
			this.name = name;
